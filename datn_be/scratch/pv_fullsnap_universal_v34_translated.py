...
VISUAL_PANEL_THICKNESS = 3
INNER_MARGIN_BASE_PX = 3
INNER_MARGIN_FACTORS = (1, 2, 3)
SAVE_INNER_MASK_DEBUG = True
DRAW_INNER_CONTOUR_DEBUG = True
...
def extract_support_points_for_horizontal_edge(rotated_gray, m_ref, c_ref, x_left, x_right, block_name, row_idx, row_count, is_outer_edge=False):
    ...
    for sx in sample_xs:
        ...
        if 0 < min_idx < len(profile) - 1:
            if profile[min_idx] < profile[min_idx - 1] and profile[min_idx] < profile[min_idx + 1]:
                ...
                if (valley_depth >= 4 or profile[min_idx] <= np.median(profile) - 3) and profile[min_idx] < 185:
    ...
    if len(clean_pts) >= max(6, int(0.20 * num_samples)):
        ...
        try:
            ...
            if coverage >= 0.45 and std_res_fit <= 2.5 and abs(center_shift) <= max_shift:
                support_valid = True
...
"""
    ...
    for r_idx, g in enumerate(snapped_horiz_gaps):
        try:
            ...
            # V34-fix: đồng thời bảo vệ chống snap vào các vùng đất/mặt đất.
            # Rãnh tấm pin thực tế có center_med >= 55 (thông thường là 80-130).
            # Vùng đất/mặt đất có center_med < 40. Ngưỡng 55 cung cấp
            # khoảng chừa an toàn để từ chối mặt đất mà không chạm vào các rãnh thực.
...
def refine_horizontal_rows_by_local_valley_anchors(rotated_gray, boundaries, snapped_horiz_gaps, x_mid, block_name,
                                                   ...
                                                   min_coverage=0.50, max_residual_std=2.6, n_rows=None):
    """
...
"""
    ...
    total_rows = n_rows if n_rows is not None else len(snapped_horiz_gaps)
    refined = []
    for r_idx, g0 in enumerate(snapped_horiz_gaps):
        g = dict(g0)
        # V34-fix: các hàng ngoài cùng (đầu tiên và cuối cùng) giáp mặt đất/bầu trời — sử dụng nghiêm ngặt hơn
        # giới hạn dịch chuyển và độ bao phủ để việc snap không bị lệch sang vùng đất tối.
        is_outer_row = (r_idx == 0 or r_idx == total_rows - 1)
        effective_max_shift_px = 1.5 if is_outer_row else max_shift_px
        effective_min_coverage = 0.65 if is_outer_row else min_coverage
        try:
            m_row = float(g.get('support_line_m') if g.get('support_line_m') is not None else g.get('m_row', 0.0))
            c_row = float(g.get('support_line_c') if g.get('support_line_c') is not None else g.get('c_row', 0.0))
            y_mid = float(m_row * x_mid + c_row)
            ...
            # V34-fix: loại bỏ các điểm neo có giá trị điểm ảnh trung vị tối ngang mức mặt đất.
            # Rãnh tấm pin thực tế: dark_med >= 55. Mặt đất: dark_med < 40.
            anchor_dark_med = float(np.median([p[2] for p in anchor_pts]))
            if anchor_dark_med < 55.0:
                # Đám mây điểm neo bị chiếm lĩnh bởi điểm ảnh mặt đất, không phải rãnh thực tế.
                # Không để nó di chuyển hàng đi sâu hơn vào vùng mặt đất.
            ...
            if abs(shift) > effective_max_shift_px:
                c_new += math.copysign(effective_max_shift_px - abs(shift), shift)
                shift = math.copysign(effective_max_shift_px, shift)
            ...
            accept = (coverage >= effective_min_coverage and residual_new <= residual_limit and
...
def build_grid_from_yolo_edge_consensus(rotated_gray, rotated_sobel_x_abs, b_local, y_min, y_max, n_cols, row_count, x_min_roi, x_max_roi, block_name, img_w=640, img_h=512):
    ...
    for r in range(row_count):
        if row_points_list[r] and row_points_list[r + 1]:
            y_r = np.mean([pt[1] for pt in row_points_list[r]])
            y_r1 = np.mean([pt[1] for pt in row_points_list[r + 1]])
            pitches.append(y_r1 - y_r)
    estimated_pitch = float(np.median(pitches)) if pitches else (y_max - y_min) / max(1, row_count)
    ...
    snapped_horiz_gaps = refine_horizontal_rows_by_local_valley_anchors(
        ...
        search_radius=V34_ROW_SEARCH_RADIUS_PX, margin_px=7.0, max_shift_px=4.0,
        n_rows=len(snapped_horiz_gaps)
    )

    # Giới hạn các hàng ngoài cùng (r = 0 và r = row_count) bằng khoảng cách ước lượng của khối
    # để ngăn chúng trôi vào vùng đất/mặt đất.
    if len(snapped_horiz_gaps) >= 2:
        min_p = 0.90 * estimated_pitch
        max_p = 1.08 * estimated_pitch
        
        # 1. Hàng trên cùng (r = 0)
        g0 = snapped_horiz_gaps[0]
        g1 = snapped_horiz_gaps[1]
        y0 = g0["y_snap"]
        y1 = g1["y_snap"]
        pitch_top = y1 - y0
        if not (min_p <= pitch_top <= max_p):
            clamped_pitch = max(min_p, min(max_p, pitch_top))
            y0_new = y1 - clamped_pitch
            shift = y0_new - y0
            for key in ("c_row", "c_selected", "support_line_c"):
                if g0.get(key) is not None:
                    g0[key] = float(g0[key] + shift)
            g0["y_snap"] = y0_new
            print(f"[OUTER_ROW_PITCH_CLAMP] block={block_name} r=0 pitch_orig={pitch_top:.2f} pitch_clamped={clamped_pitch:.2f} shift={shift:.2f}")
            
        # 2. Hàng dưới cùng (r = row_count)
        g_last = snapped_horiz_gaps[-1]
        g_prev = snapped_horiz_gaps[-2]
        y_last = g_last["y_snap"]
        y_prev = g_prev["y_snap"]
        pitch_bot = y_last - y_prev
        if not (min_p <= pitch_bot <= max_p):
            clamped_pitch = max(min_p, min(max_p, pitch_bot))
            y_last_new = y_prev + clamped_pitch
            shift = y_last_new - y_last
            for key in ("c_row", "c_selected", "support_line_c"):
                if g_last.get(key) is not None:
                    g_last[key] = float(g_last[key] + shift)
            g_last["y_snap"] = y_last_new
            print(f"[OUTER_ROW_PITCH_CLAMP] block={block_name} r={row_count} pitch_orig={pitch_bot:.2f} pitch_clamped={clamped_pitch:.2f} shift={shift:.2f}")

    # V34-fix: Đảm bảo tối thiểu theo YOLO cho các hàng biên ngoài.
    # Yêu cầu: tấm pin snap-grid KHÔNG được nhỏ hơn đa giác YOLO.
    # - đường r=0 phải ở tại hoặc TRÊN cạnh trên YOLO (y_snap[0] <= y_r0_yolo).
    #   Nếu y_snap[0] > y_r0_yolo tấm pin vẽ ra bị ngắn hơn ở đỉnh so với YOLO -> sửa.
    # - đường r=cuối phải ở tại hoặc DƯỚI cạnh dưới YOLO (y_snap[-1] >= y_rlast_yolo).
    #   Nếu y_snap[-1] < y_rlast_yolo tấm pin vẽ ra bị ngắn hơn ở đáy so với YOLO -> sửa.
    # Chúng tôi cũng giới hạn mức độ cho phép ở TRÊN/DƯỚI YOLO (OUTER_ROW_YOLO_SLACK_PX) để ngăn chặn
    # snap khỏi việc đi quá nhiều vào vùng đất trong khi vẫn thỏa mãn quy tắc kích thước tối thiểu.
    OUTER_ROW_YOLO_SLACK_PX = 3.0   # snap có thể vượt quá YOLO tối đa ngần này pixel
    if len(snapped_horiz_gaps) >= 1 and len(row_points_list) >= 1:
        # --- r = 0: snap không được nằm dưới cạnh trên YOLO ---
        if row_points_list[0]:
            ...
            # Phạm vi cho phép: [y_r0_yolo - SLACK, y_r0_yolo]
            # Giới hạn dưới: snap không được cao hơn YOLO top một lượng quá SLACK (ngăn đi quá xa lên trời/đất phía trên)
            # Giới hạn trên: snap không được nằm dưới cạnh trên YOLO (nếu không tấm pin sẽ nhỏ hơn YOLO)
            y0_allowed_lo = y_r0_yolo - OUTER_ROW_YOLO_SLACK_PX
            y0_allowed_hi = y_r0_yolo  # giới hạn cứng: không được thấp hơn đỉnh YOLO
            if not (y0_allowed_lo <= y0_cur <= y0_allowed_hi):
                y0_new = max(y0_allowed_lo, min(y0_allowed_hi, y0_cur))
        ...
        # --- r = row_count: snap không được nằm trên cạnh dưới YOLO ---
        if len(snapped_horiz_gaps) >= 1 and len(row_points_list) >= row_count + 1 and row_points_list[row_count]:
            ...
            # Phạm vi cho phép: [y_rlast_yolo, y_rlast_yolo + SLACK]
            # Giới hạn dưới: snap không được ở TRÊN đáy YOLO (tấm pin nhỏ hơn YOLO ở phía đáy)
            # Giới hạn trên: snap có thể vượt tối đa SLACK xuống dưới đáy YOLO (ngăn đi quá sâu vào vùng đất)
            y_last_allowed_lo = y_rlast_yolo  # giới hạn cứng: không được cao hơn đáy YOLO
            y_last_allowed_hi = y_rlast_yolo + OUTER_ROW_YOLO_SLACK_PX
            if not (y_last_allowed_lo <= y_last_cur <= y_last_allowed_hi):
                y_last_new = max(y_last_allowed_lo, min(y_last_allowed_hi, y_last_cur))
    ...
    if has_crop_left_col:
        ...
        for r in range(row_count + 1):
            for col_idx in range(n_cols):
                ...
                if n_cols == 2:
                    ...
                    x0, x1 = (x_left_outer + 8, x_right_outer - 8)
                ...
                if n_cols == 2:
                    ...
                    x0_fit = x_left_outer + 6
                    x1_fit = x_right_outer - 6
        ...
        if has_crop_left_col:
            ...
            cols_to_process = [1, 0] if n_cols == 2 else [0]
                ...
                for r in range(row_count + 1):
                    ...
                    if neighbor_r is not None:
                        ...
                        if n_cols == 2:
                            ...
                            x_segment_left = x_left_outer
                            x_segment_right = x_right_outer
        ...
        for r in range(row_count + 1):
            gaps_to_check = [left_snapped_horiz_gaps_by_col[c][r] for c in range(n_cols)]
            for col_idx, g in enumerate(gaps_to_check):
                if g["selected_cand"] in ("crop_left_support_points_line", "crop_left_support_fit", "crop_left_borrow_right_line", "crop_left_neighbor_inherit", "crop_left_borrow_same_row_right", "crop_left_pitch_extrapolate"):
                    ...
                    continue
                # Chỉ chuẩn hóa các đường dự phòng (dy_fallback / yolo) nếu có 2 cột
                if n_cols == 2:
                    g_other = gaps_to_check[1] if col_idx == 0 else gaps_to_check[0]
            ...
            for r in range(row_count + 1):
                for col_idx in range(n_cols):
                    ...
                    if n_cols == 2:
                        ...
                        left_boundary = boundaries["left_outer"]
                        right_boundary = boundaries["right_outer"]
        ...
        # Giới hạn các đường biên ngoài crop-left bằng cách sử dụng khoảng cách ước lượng của cột.
        if left_snapped_horiz_gaps_by_col:
            ...
            for col_idx in left_snapped_horiz_gaps_by_col:
                ...
                if 0 in gaps and 1 in gaps and row_count in gaps and (row_count - 1) in gaps:
                    # 1. Hàng trên cùng (r = 0)
                    g0 = gaps[0]
                    g1 = gaps[1]
                    y0 = g0["y_snap"]
                    ...
                    if not (min_p <= pitch_top <= max_p):
                        ...
                        y0_new = y1 - clamped_pitch
                        shift = y0_new - y0
                        for key in ("c_row", "support_line_c", "c_final"):
                            if g0.get(key) is not None:
                                g0[key] = float(g0[key] + shift)
                        g0["y_snap"] = y0_new
                        print(f"[CROP_LEFT_OUTER_ROW_PITCH_CLAMP] block={block_name} col={col_idx} r=0 pitch_orig={pitch_top:.2f} pitch_clamped={clamped_pitch:.2f} shift={shift:.2f}")

                    # 2. Hàng dưới cùng (r = row_count)
                    g_last = gaps[row_count]
                    g_prev = gaps[row_count - 1]
                    y_last = g_last["y_snap"]
                    y_prev = g_prev["y_snap"]
                    pitch_bot = y_last - y_prev
                    if not (min_p <= pitch_bot <= max_p):
                        ...
                        for key in ("c_row", "support_line_c", "c_final"):
                            if g_last.get(key) is not None:
                                g_last[key] = float(g_last[key] + shift)
                        g_last["y_snap"] = y_last_new
                        print(f"[CROP_LEFT_OUTER_ROW_PITCH_CLAMP] block={block_name} col={col_idx} r={row_count} pitch_orig={pitch_bot:.2f} pitch_clamped={clamped_pitch:.2f} shift={shift:.2f}")
