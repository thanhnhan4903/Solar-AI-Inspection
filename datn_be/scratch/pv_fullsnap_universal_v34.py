...
def refine_horizontal_rows_to_dark_groove(rotated_gray, boundaries, snapped_horiz_gaps, x_mid, block_name,
    ...
    for r_idx, g in enumerate(snapped_horiz_gaps):
        try:
            ...
            # V34-fix: also guard against snapping into earth/ground regions.
            # Real panel grooves have center_med >= 55 (typically 80-130).
            # Earth/ground regions have center_med < 40. Threshold 55 gives
            # safe headroom to reject earth without touching real grooves.
...
def refine_horizontal_rows_by_local_valley_anchors(rotated_gray, boundaries, snapped_horiz_gaps, x_mid, block_name,
    ...
    for r_idx, g0 in enumerate(snapped_horiz_gaps):
        g = dict(g0)
        # V34-fix: outer rows (first and last) border the ground/sky — use stricter
        # shift and coverage limits so the snap does not drift onto dark earth.
        ...
        try:
            ...
            # V34-fix: reject anchors whose median pixel value is earth-level dark.
            # Real panel grooves: dark_med >= 55. Earth/ground: dark_med < 40.
            anchor_dark_med = float(np.median([p[2] for p in anchor_pts]))
            if anchor_dark_med < 55.0:
                # The anchor cloud is dominated by earth pixels, not a real groove.
                # Do not let it move the row further into the ground.
...
def build_grid_from_yolo_edge_consensus(rotated_gray, rotated_sobel_x_abs, b_local, y_min, y_max, n_cols, row_count, x_min_roi, x_max_roi, block_name, img_w=640, img_h=512):
    ...
    # Clamp outer rows (r = 0 and r = row_count) using estimated pitch of the block
    # to prevent them from drifting into the ground/earth.
    ...
    # V34-fix: YOLO-minimum guarantee for outer rows.
    # Requirement: the snap-grid panel must NOT be smaller than the YOLO polygon.
    # - r=0 line must be at or ABOVE the YOLO top edge (y_snap[0] <= y_r0_yolo).
    #   If y_snap[0] > y_r0_yolo the drawn panel is shorter on top than YOLO -> fix.
    # - r=last line must be at or BELOW the YOLO bottom edge (y_snap[-1] >= y_rlast_yolo).
    #   If y_snap[-1] < y_rlast_yolo the drawn panel is shorter on bottom than YOLO -> fix.
    # We also cap how far ABOVE/BELOW YOLO we allow (OUTER_ROW_YOLO_SLACK_PX) to prevent
    # snap from going excessively into ground while still satisfying the minimum-size rule.
    OUTER_ROW_YOLO_SLACK_PX = 3.0   # snap may exceed YOLO by at most this many px
    if len(snapped_horiz_gaps) >= 1 and len(row_points_list) >= 1:
        # --- r = 0: snap must not be below YOLO top edge ---
        if row_points_list[0]:
            ...
            # Allowed range: [y_r0_yolo - SLACK, y_r0_yolo]
            # Lower bound: snap must not be more than SLACK above YOLO top (prevent going too far into sky/ground above)
            # Upper bound: snap must not be BELOW YOLO top edge (panel would be smaller than YOLO)
            y0_allowed_lo = y_r0_yolo - OUTER_ROW_YOLO_SLACK_PX
            y0_allowed_hi = y_r0_yolo  # hard cap: cannot be lower than YOLO top
        ...
        # --- r = row_count: snap must not be above YOLO bottom edge ---
        if len(snapped_horiz_gaps) >= 1 and len(row_points_list) >= row_count + 1 and row_points_list[row_count]:
            ...
            # Allowed range: [y_rlast_yolo, y_rlast_yolo + SLACK]
            # Lower bound: snap must not be ABOVE YOLO bottom (panel smaller than YOLO on bottom)
            # Upper bound: snap may go at most SLACK below YOLO bottom (prevent going too far into ground)
            y_last_allowed_lo = y_rlast_yolo  # hard cap: cannot be higher than YOLO bottom
    ...
    if has_crop_left_col:
        ...
        for r in range(row_count + 1):
            ...
            for col_idx, g in enumerate(gaps_to_check):
                if g["selected_cand"] in ("crop_left_support_points_line", "crop_left_support_fit", "crop_left_borrow_right_line", "crop_left_neighbor_inherit", "crop_left_borrow_same_row_right", "crop_left_pitch_extrapolate"):
                    ...
                    continue
                # Chỉ regularize fallback lines (dy_fallback / yolo) nếu có 2 cột
                if n_cols == 2:
        ...
        # Clamp outer crop-left lines using estimated pitch of the column.
