...
def build_grid_from_yolo_edge_consensus(rotated_gray, rotated_sobel_x_abs, b_local, y_min, y_max, n_cols, row_count, x_min_roi, x_max_roi, block_name):
    ...
    if IMAGE_STEM == "DJI_0087_R" and has_crop_left_col:
        ...
        crop_debug_img = None
        jsonl_lines = []
        if IMAGE_STEM == "DJI_0087_R" and has_crop_left_col and block_name == "block_1":
        ...
        if IMAGE_STEM == "DJI_0087_R" and block_name == "block_1":
        ...
        if crop_debug_img is not None:
            out_dir = Path("data/results/debug")
            out_dir.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(out_dir / "debug_DJI_0087_R_crop_left_snap_debug.JPG"), crop_debug_img)
            
            jsonl_path = out_dir / "debug_DJI_0087_R_crop_left_profiles.jsonl"
...
def refine_small_block_panels_individually(rotated_gray, b_local, block_name, img_support, img_snap, transform_local_to_orig, transform_orig_to_local):
    ...
    is_lower_small_block = (
        IMAGE_STEM == "DJI_0845_R" and
        (row_count <= 2 or len(b_local["panels"]) <= 4) and
        block_name in ("block_3", "block_5", "block_7")
    )
    has_real_lower_grid = False

    if IMAGE_STEM == "DJI_0845_R" and block_name in ("block_3", "block_5", "block_7"):
        ...
        if block_name == "block_7":
            # YC1: log block_7 route active
            print(f"[BLOCK7_ROUTE] block=block_7 active=true")
...
def main():
    ...
    global GLOBAL_LOWER_SUPPORT_DEBUG_IMG
    if IMAGE_STEM == "DJI_0845_R":
        GLOBAL_LOWER_SUPPORT_DEBUG_IMG = img.copy()
    else:
        GLOBAL_LOWER_SUPPORT_DEBUG_IMG = None
    ...
    for temp_idx, temp_b in enumerate(blocks):
        ...
        is_small = (IMAGE_STEM == "DJI_0845_R" and (temp_row_count < 4 or len(temp_panels) <= 4))
        if not is_small:
    ...
    if GLOBAL_LOWER_SUPPORT_DEBUG_IMG is not None:
        cv2.imwrite(str(out_dir / f"debug_{IMAGE_STEM}_lower_support_debug.JPG"), GLOBAL_LOWER_SUPPORT_DEBUG_IMG)
